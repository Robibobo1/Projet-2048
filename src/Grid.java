	...
	public void move_left_right(int direction) {
		...
		for (int x = 0; x < size; x++) // lignes
		{
			for (int y = 0; y < size; y++) // colonnes
			{
				...
				{
					// on va lire toutes les colonnes qui pr�c�dent ce bloc, et si on d�tecte
					// du vide ou un bloc semblable, on effectue un d�calage
					...
					{
						if (blocTab[idx][l] == blocTab[c][l]) // si le dernier bloc a la m�me valeur que le bloc
																// actuel
						{
		...
		for (int x = 0; l < size; x++) // lignes
		{
			for (int y = 1; y < size; y++) // colonnes
			{
				...
				if (blocTab[l][c] != -1) // si la case contient un bloc
				{
					...
					if (c != idx)// si on a au moins boug� d'une case
					{
						if (blocTab[l][idx] == blocTab[l][c]) // si le dernier bloc a la m�me valeur que le bloc
																// actuel
						{
		...
		for (int l = 0; l < size; l++) // lignes
		{
			for (int c = 1; c < size; c++) // colonnes
			{
				...
				if (blocTab[l][c] != -1) // si la case contient un bloc
				{
					...
					{
						if (blocTab[l][idx + 1] == blocTab[l][c]) // si le dernier bloc a la m�me valeur que le bloc
																	// actuel
						{
